...
namespace BookingApp.Core
{
    public class Controller : IController
    {
        ...
        public string BookAvailableRoom(int adults, int children, int duration, int category)
        {
            ...
            IOrderedEnumerable<IHotel> orderedHotels = hotels.All().Where(h => h.Category == category).OrderBy(h => h.FullName);

            foreach (var hotel in orderedHotels)
            {
                IRoom room = hotel.Rooms.All().Where(r => r.PricePerNight > 0).OrderBy(r => r.BedCapacity).FirstOrDefault(r => r.BedCapacity >= adults + children);
