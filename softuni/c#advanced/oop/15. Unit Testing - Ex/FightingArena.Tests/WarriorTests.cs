namespace FightingArena.Tests
{
    ...
    public class WarriorTests
    {
        ...
        [TestCase(null)]
        [TestCase(" ")]
        public void WarriorShouldThrowIfNullOrWhiteSpace(string name)
        {
            ArgumentException exeption = Assert.Throws<ArgumentException>(() => new Warrior(name, 15, 45));
        ...
        {
            ArgumentException exeption = Assert.Throws<ArgumentException>(() => new Warrior("Pesho", damage, 45));
            Assert.That(exeption.Message, Is.EqualTo("Damage value should be positive!"));
        }

        [Test]
        public void WarriorShouldThrowIfHpIsLessThan0()
        {
            ArgumentException exeption = Assert.Throws<ArgumentException>(() => new Warrior("Pesho", 15, -45));
            Assert.That(exeption.Message, Is.EqualTo("HP should not be negative!"));
        }

        [Test]
        public void AttackShouldThrowIfHpIsLessThan30()
        {
            var attacker = new Warrior("Gosho", 10, 20);
            InvalidOperationException exeption = Assert.Throws<InvalidOperationException>(() => attacker.Attack(warrior));
        ...
        public void AttackShouldThrowIfDefendersHPLessThan30()
        {
            ...
            InvalidOperationException exeption = Assert.Throws<InvalidOperationException>(() => warrior.Attack(defender));
